...
import { FluytNavigationItem, FluytVerticalNavigationAppearance, FluytVerticalNavigationMode, FluytVerticalNavigationPosition } from 'fluyt/components/navigation/navigation.types';
...
@Component({
    selector: 'fluyt-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: fluytAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'fluytVerticalNavigation'
})
export class FluytVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy {
    ...
    @Input() appearance: FluytVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: FluytVerticalNavigationMode = 'side';
    @Input() name: string = this._fluytUtilsService.randomId();
    @Input() navigation: FluytNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: FluytVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<FluytVerticalNavigationAppearance> = new EventEmitter<FluytVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<FluytVerticalNavigationMode> = new EventEmitter<FluytVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<FluytVerticalNavigationPosition> = new EventEmitter<FluytVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<FluytNavigationItem> = new ReplaySubject<FluytNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<FluytNavigationItem> = new ReplaySubject<FluytNavigationItem>(1);
    ...
    private _fluytScrollbarDirectives!: QueryList<FluytScrollbarDirective>;
    ...
    @HostBinding('class') get classList(): any {
        return {
            ...
            [`fluyt-vertical-navigation-appearance-${this.appearance}`]: true,
            'fluyt-vertical-navigation-hover': this._hovered,
            'fluyt-vertical-navigation-inner': this.inner,
            'fluyt-vertical-navigation-mode-over': this.mode === 'over',
            'fluyt-vertical-navigation-mode-side': this.mode === 'side',
            'fluyt-vertical-navigation-opened': this.opened,
            'fluyt-vertical-navigation-position-left': this.position === 'left',
            'fluyt-vertical-navigation-position-right': this.position === 'right'
        };
    }
    ...
    /**
     * Setter for fluytScrollbarDirectives
     */
    @ViewChildren(FluytScrollbarDirective)
    set fluytScrollbarDirectives(fluytScrollbarDirectives: QueryList<FluytScrollbarDirective>) {
        // Store the directives
        this._fluytScrollbarDirectives = fluytScrollbarDirectives;

        // Return if there are no directives
        if (fluytScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._fluytScrollbarDirectivesSubscription) {
            this._fluytScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._fluytScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() => {
                    ...
                    fluytScrollbarDirectives.forEach((fluytScrollbarDirective) => {
                        fluytScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void {
        setTimeout(() => {
            ...
            if (!this._navigationContentEl.nativeElement.classList.contains('ps')) {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.fluyt-vertical-navigation-item-active');
            ...
            // Otherwise
                ...
                this._fluytScrollbarDirectives.forEach((fluytScrollbarDirective) => {

                    // Skip if not enabled
                    if (!fluytScrollbarDirective.isEnabled()) {
                        return;
                    }

                    // Scroll to the active element
                    fluytScrollbarDirective.scrollToElement('.fluyt-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: FluytNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: FluytNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('fluyt-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add('fluyt-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void {
        ...
        this._asideOverlay.classList.add('fluyt-vertical-navigation-aside-overlay');
